...
class SecondarySiteFinder:
    ...
    @staticmethod
    def load_sites_from_fasta(fasta_path: str) -> Dict[str, List[Tuple[str, float]]]:
        """
        Loads secondary binding sites from a FASTA file using BioPython's SeqIO.

        The FASTA file is expected to be in the format generated by the `find_sites` method.
        Each header for a mutated sequence should contain 'ddG_binding'. Reference sequences are ignored.

        Args:
            fasta_path (str): Path to the input FASTA file.

        Returns:
            Dict[str, List[Tuple[str, float]]]: A dictionary mapping target_id to a list of
                                                 (mutation_sequence, ddG_binding) tuples.
        """
        if not os.path.exists(fasta_path):
            logging.error(f"FASTA file not found: {fasta_path}")
            raise FileNotFoundError(f"FASTA file not found: {fasta_path}")

        secondary_sites: Dict[str, List[Tuple[str, float]]] = defaultdict(list)

        with open(fasta_path, 'r') as f:
            for record in SeqIO.parse(f, "fasta"):
                header = record.description
                if "(reference)" in header or "ddG_binding" not in header:
                    continue

                try:
                    # record.id is the part of the header before the first space
                    header_main_part = record.id
                    
                    # Robustly find the base target_id by removing the numeric suffix
                    match = re.match(r'(.+)_(\d+)$', header_main_part)
                    if not match:
                        logging.warning(f"Could not parse target_id and index from header: {header}")
                        continue
                    target_id = match.group(1)
                    
                    ddg_match = re.search(r'ddG_binding=([-\d\.]+)', header)
                    if not ddg_match:
                        logging.warning(f"Could not parse ddG_binding from header: {header}")
                        continue
                    
                    ddg_binding = float(ddg_match.group(1))
                    sequence = str(record.seq)
                        
                    secondary_sites[target_id].append((sequence, ddg_binding))

                except Exception as e:
                    logging.warning(f"Skipping record due to parsing error in header '{header}': {e}")
        
        return dict(secondary_sites)
